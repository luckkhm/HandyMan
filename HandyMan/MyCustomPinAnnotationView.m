...
- (instancetype)initWithAnnotation:(id<MKAnnotation>)annotation andidHotel:(NSString *)idHotel
{
    ...
    _pointAnnotation = (MyCustomPointAnnotation*) annotation;
        ...
        NSLog(@"self.idHotel %@",self.idHotel);
        _pointAnnotation.idHotel = idHotel;
    ...
    // Hotel Nom
    UILabel *labelHotel = [[UILabel alloc]initWithFrame:CGRectMake(10, 10, 180, 21)];
    labelHotel.text = [annotation title];
    labelHotel.textAlignment = NSTextAlignmentCenter;
    labelHotel.adjustsFontSizeToFitWidth = YES;
    labelHotel.textColor = [UIColor whiteColor];
    [customview addSubview:labelHotel];
    
    // Hotel adress
    UILabel *labelHotelAdress = [[UILabel alloc]initWithFrame:CGRectMake(8, 25, 194, 21)];
    labelHotelAdress.text = [annotation subtitle];
    labelHotelAdress.adjustsFontSizeToFitWidth = YES;
    labelHotelAdress.textColor = [UIColor grayColor];
    [customview addSubview:labelHotelAdress];
...
-(void)clicktoshow{
    [self.delegate ShowHotelDetails:self.pointAnnotation.idHotel];
}
